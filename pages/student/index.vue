    ...
    <div class="absolute top-24 w-full h-full my-10">
      <div class="relative bg-white shadow-md rounded-lg p-5 mx-7">
        ...
        <div class="flex flex-wrap w-full justify-between p-3">
          ...
          >
            ...
            <div class="flex flex-col py-5 pl-24">
              ...
              <span class="text-slate-500">{{ totalPresentLate }} Day</span>
            </div>
          </div>
        </div>
        <div class="absolute top-2.5 right-0">
          <div class="relative">
            <button
              class="absolute top-1 right-0 cursor-pointer bg-[#F7931E] text-white w-11 inline-flex gap-5 truncate px-3 p-2.5 rounded-l-full duration-300 hover:duration-300 hover:w-36 hover:gap-3"
      ...
      >
        <div class="text-center space-y-7 w-5/12 py-5">
          ...
          <div class="flex flex-row justify-between gap-6 mt-7">
            ...
            <div class="text-center flex flex-col justify-center gap-y-5 w-1/4">
              <div class="flex justify-center">
                <span class="rounded-full bg-[#F7931E] p-4"><Togahat /></span>
              </div>
              <p class="flex flex-col">
      ...
      <div class="h-fit w-full bg-gray-100 flex justify-center py-7">
        <div class="text-center space-y-7 w-8/12">
          <div class="space-y-2">
            ...
            <span class="flex justify-center"><Bordercustom /> </span>
          </div>
            ...
            >
              <div class="flex justify-center">
                <span class="border rounded-full bg-[#F7931E] p-4">
                  <Quotesquare class="fill-[#FFFFFF]" />
                </span>
              </div>
              <p class="text-justify flex flex-col">
                <span>
                  <Quote class="float-left -ml-2 mr-1 fill-[#F7931E]" />
...
import Bordercustom from '~/components/icons/bordercustom.vue';
import Envelop from '~/components/icons/envelop.vue';
import In from '~/components/icons/in.vue';
import Quote from '~/components/icons/quote.vue';
import Quotesquare from '~/components/icons/quotesquare.vue';
import Sad from '~/components/icons/sad.vue';
import Stopwatch from '~/components/icons/stopwatch.vue';
import Togahat from '~/components/icons/togahat.vue';
  ...
  methods: {
    ...
    async fetchLastAbsent() {
      try {
        ...
        if (res.data === null) {
          this.lastAbsent = 'Absent Yet';
        } else {
          this.lastAbsent = res.data.dateSubmit;
        }
      } catch (error) {
        console.log(error);
        // eslint-disable-next-line new-cap
